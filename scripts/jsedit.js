...
/****
if (location.href == "http://kozikfamily.net/" ) {
    JS.jefileRestURI = "http://kozikfamily.net/rest/";
} else if (location.href == "https://cisco163.kozikfamily.net/" ) {
...
/* Load Chart of Accounts File for current year */
    ...
    //$.blockUI({ message: '<h2>Initialing ...</h2>' });
    //$.blockUI({ message: '<h3><img src="images/busy.gif" />Initialing ...</h3>',
    ...
    $.ajax({"url":chartRestGET,
        ...
        success: function(response) {
            ...
            */
            ...
            /* Logging */
            $("#contentJournal").append("<br>... received "+response.length+ " bytes");
            JS.logHtml += "<br>... received "+response.length+ " bytes";

            var chsplit = new Array();
            var chartfile = response.split("<br>");
            $(chartfile).each( function() {
                chsplit = this.split(',');
                if( chsplit[4] !== undefined && chsplit[4].substring(0,1) == "s" ) {
                    JS.chart[chsplit[1]] = {"chDesc":chsplit[2],"subacct":true};
                } else {
                    JS.chart[chsplit[1]] = {"chDesc":chsplit[2],"subacct":false};
                }
            });

            /* Logging */
            $("#contentJournal").append("<br>... parsed into "+JS.chart.length+ " enteries in JS.chart[]");
            JS.logHtml += "<br>... parsed into "+JS.chart.length+ " enteries in JS.chart[]";
            end = new Date().getTime();
            diff = end - start;
            $("#contentJournal").append("<br>... time "+ diff);
...
** Output: JS.nfyy content array, keys: "01.pgl" through "12.pgl"
**
** Each Journal Entry JS.nfyy["01" - "12"].jefile[i] is formated from
** - 0131,-621,75.60;323-,Mobil
** The GL records are stored as JS.nfyy["01.pgl" - "12.pgl"].pglfile[i]
...
** - Input: JS.nfyy["01.pgl" - "12.pgl"].pglfile[i] 
